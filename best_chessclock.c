...
void displayTime(unsigned int time, unsigned char row, unsigned char col) {
    ...
    LCD_print((unsigned char*)timeStr);
}

void obsluga_przyciskow(unsigned char* currentPlayer, unsigned char* gameActive) {
    if (BUTTON_IsPressed(BUTTON_S4)) {
        __delay_ms(300); // debouncing delay
        while (BUTTON_IsPressed(BUTTON_S4)); // wait until button is released
        *gameActive = !(*gameActive);
        if (!(*gameActive)) {
            LCD_sendCommand(LCD_CLEAR);
            LCD_setCursor(1, 0);
            LCD_print((unsigned char*)"Game Stopped");
        }
    }
...
void chessClock() {
    ...
    while (1) {
        obsluga_przyciskow(&currentPlayer, &gameActive);
